...
async function fetchCustomers() {
  ...
  // The API paginates (has_more/next_cursor) — a single fetch only ever
  // returns the first page (200 rows by default), so a full sync must
  // keep following next_cursor until has_more is false, or older/lower
  // pages silently never make it into the CRM.
